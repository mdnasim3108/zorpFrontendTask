...
const MobileNav = (props) => {
  ...
  const [set, showSet] = useState(false);
  return (
    <div className="w-full h-full  lg:hidden block">
      <div
        className={`fixed  flex bg-white ${
          set ? "w-[70%]" : "w-[18%]"
        } h-full z-20  border-r-gray-200 ${
          !showNav ? "left-[-999px]" : "left-0"
        } transition-all duration-300 ease-in-out`}
      >
        <div
          className={` ${
            !set ? "w-full" : "w-[20%] border-r-gray-300"
          }  relative  border-r-2  h-full flex flex-col items-center justify-between  py-3`}
        >
        ...
        {set && <FontAwesomeIcon
          icon={faCircleXmark}
          className="absolute top-5 right-5 text-gray-500 w-[2rem] h-[2rem]"
      ...
      {showNav && (
        <div
          className="bg-black opacity-[0.5] w-full h-[200vh] absolute z-10"
